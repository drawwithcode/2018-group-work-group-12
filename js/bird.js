...
renderer.setClearColor(0xd8d8ce, 1.0);
renderer.setPixelRatio(window.devicePixelRatio || 1);
$viewport.appendChild(renderer.domElement);

const camera = new THREE.PerspectiveCamera(25, 1, 1, 80);
camera.position.z = 4;
...
const material = new THREE.RawShaderMaterial({
  vertexShader: `
	...
	void main() {
		...
		float totalTime = time + timeOffset;
...
const offsets = new Float32Array(BIRDS_COUNT * 3);

for (let i = 0; i < offsets.length; i += 3) {
  offsets[i] = (Math.random() * 2 - 1) * 5;
  offsets[i + 1] = (Math.random() * 2 - 1) * 5;
  offsets[i + 2] = (Math.random() * 2 - 1) * 5;
}
...
const colors = new Float32Array(BIRDS_COUNT * 3);

for (let i = 0; i < colors.length; ++i) {
  colors[i] = Math.random() * 0.075 + 0.65;
}

geometry.addAttribute('position', new THREE.BufferAttribute(basePositions, 3));
geometry.addAttribute(
  'highPosition',
  new THREE.BufferAttribute(highPositions, 3)
);
geometry.addAttribute('uv', new THREE.BufferAttribute(baseUvs, 2));
geometry.addAttribute(
  'offset',
  new THREE.InstancedBufferAttribute(offsets, 3, 1).setDynamic(true)
);
geometry.addAttribute(
  'timeOffset',
  new THREE.InstancedBufferAttribute(timeOffsets, 1, 1)
);
geometry.addAttribute(
  'scale',
  new THREE.InstancedBufferAttribute(scales, 1, 1)
);
geometry.addAttribute(
  'color',
  new THREE.InstancedBufferAttribute(colors, 3, 1)
);
