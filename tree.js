...
function draw() {
  ...
  for (i = 1; i < count; i++) {
    strokeWeight(
      myTree.twig[parseInt(myTree.map[i].x)].thickness[
        parseInt(myTree.map[i].y)
      ]
    );
    line(
      myTree.twig[parseInt(myTree.map[i].x)].location[
        parseInt(myTree.map[i].y) - 1
      ].x,
      myTree.twig[parseInt(myTree.map[i].x)].location[
        parseInt(myTree.map[i].y) - 1
      ].y,
      myTree.twig[parseInt(myTree.map[i].x)].location[parseInt(myTree.map[i].y)]
        .x,
      myTree.twig[parseInt(myTree.map[i].x)].location[parseInt(myTree.map[i].y)]
        .y
    );
  ...
  for (i = 0; i < myTree.twig.length; i++) {
    ...
    ellipse(
      myTree.twig[i].location[num].x,
      myTree.twig[i].location[num].y,
      0,
      0
    );
  }
...
function Tree(startPoint, direction) {
          ...
          if (this.fr[id].thickness > BranchLocationFactor) {
            ...
            if (_id !== 0) {
              for (var _i = 0; _i < 2; _i++) {
                this.twig[this.twig.length - 1].baseIndex[_i] = concat(
                  this.twig[this.twig.length - 1].baseIndex[_i],
  ...
  for (i = 1; i < this.candNum; i++) {
    ...
    this.twig[this.candidateIndex[i]].dTheta = Array(
  ...
  for (var i = 0; i < this.candNum; i++) {
    ...
    for (j = 0; j < _num; j++)
      this.twig[this.candidateIndex[i]].dTheta[j] =
        ...
        this.freq *
        cos(
          TWO_PI * this.freq * this.time -
            (this.phaseFactor[i] * PI * parseFloat(j)) / parseFloat(_num)
        );
  }
